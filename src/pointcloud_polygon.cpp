 ...
 */
PointcloudPolygon::Polygon3D::ConstPtr
PointcloudPolygon::Add(const pcl::PointCloud<pcl::PointXYZI>::Ptr &pc,
  ...
  if (plane_pc_num != indices.size()) {
    ...
    std::cout << "\033[1;32mPointCloudExtract\033[0m:\t"
              << "current plane points number : " << plane_pc_num << std::endl;
  }
...
void PointcloudPolygon::GetPlane(
    const pcl::PointCloud<pcl::PointXYZI>::Ptr &pc, const Indices &indices,
    pcl::PointCloud<pcl::PointXYZI>::Ptr &pc_plane) {
...
void PointcloudPolygon::SectorFilter(const PCI &pc, Indices &indices) {
  ...
  for (uint32_t i = 0; i < pc.points.size(); i++) {
    ...
    if (p.z > z_min_threshold && p.z < z_max_threshold &&
        dis < filter_max_distance_ && dis > filter_min_distance_ &&
...
bool PointcloudPolygon::ExtractPlane(
    const PCI &pc, Indices &indices,
    Eigen::Vector4d &coef) { // extract point cloud's polygon
  ...
  if (ids.indices.size() != 0) {
    coef << model_coef.values[0], model_coef.values[1], model_coef.values[2],
        model_coef.values[3];
