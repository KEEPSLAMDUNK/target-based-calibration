...
MainWindow::MainWindow(int argc, char **argv, QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), is_calibrated_(false),
      sid_(INT_MAX), img_(nullptr), pc_(nullptr), data_reader_(nullptr),
      is_selecting(false), is_select_end(false) {
  ui->setupUi(this);
  T_ = Eigen::Matrix4d::Identity();
  if (argc < 2) {
    QMessageBox::warning(
        this, tr("Warn"),
        tr("You need to open the config and dataset manually "));
  } else {
...
void MainWindow::on_actionOpen_Dataset_triggered() {
  if (config_path_.toStdString() ==
      std::string()) // config file hasn't been opened.
  {
  ...
  if (is_calibrated_) {
    ...
    if (!sensor_data_.back().pc_valid) // ensure that board point cloud is valid
    {
      QMessageBox::warning(this, tr("Error"),
                           tr("Current pc data is empty, adjust it!"));
      return;
    }

    // extract corners of pc plane
    pcl::PointCloud<pcl::PointXYZ>::Ptr pc_plane_cloud(
        new pcl::PointCloud<pcl::PointXYZ>);
    pcl::copyPointCloud(*sensor_data_.back().pc_plane, *pc_plane_cloud);
    Eigen::Matrix4d tf_lidar_to_target =
        extractPCFeature(pc_plane_cloud, sensor_data_.back().pc_plane_coef,
                         data_reader_->tag_size_);
    Eigen::Matrix4d corners_target;
    corners_target << 0, -data_reader_->tag_size_[1] / 2,
        -data_reader_->tag_size_[0] / 2, 1, 0, -data_reader_->tag_size_[1] / 2,
        data_reader_->tag_size_[0] / 2, 1, 0, data_reader_->tag_size_[1] / 2,
        data_reader_->tag_size_[0] / 2, 1, 0, data_reader_->tag_size_[1] / 2,
        -data_reader_->tag_size_[0] / 2, 1;
    Eigen::Matrix4d corners_lidar =
        tf_lidar_to_target.inverse() * corners_target.transpose();
    // std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>>
    // corners_lidar_vector;
...
void MainWindow::on_calibrate_clicked() {
  ...
  if (valid_data_ids.empty()) {
    QMessageBox::warning(
        this, tr("Error"),
        tr("Not enough valid data to do calibration, at lease 1"));
    return;
  }
  setEnabledAll(false);
  ...
  for (const auto i : valid_data_ids) {
    ...
    points_3d_tmp.resize(4,
                         points_3d.cols() + sensor_data_[i].corners_3d.size());
    auto &p3ds = sensor_data_[i].corners_3d;
    points_3d_tmp << points_3d, p3ds[0], p3ds[1], p3ds[2], p3ds[3];
    points_3d = points_3d_tmp;
    // points_3d_tmp.insert(points_3d_tmp.end(),
    // sensor_data_[i].corners_3d.begin(), sensor_data_[i].corners_3d.end() );
  ...
  T_ = solvePnP(points_2d, points_3d, fx, fy, cx, cy, false);
  cout << "\033[1;32mCalibrationResult\033[0m:\t"
       << "Calibration is finished with " << valid_data_ids.size()
       << " pairs of data." << endl;
  cout << "\033[1;32mCalibrationResult\033[0m:\t"
       << "The calibrated transformation from lidar to camera is:" << endl;
  cout << "Transformation matrix: \n" << T_.matrix() << endl;
  Eigen::Matrix3d rot_m_l2c = T_.topLeftCorner(3, 3);
  cout << "Qaternion(qx qy qz qw):\t"
       << Eigen::Quaterniond(rot_m_l2c).coeffs().transpose() << endl;
  cout << "Translation(tx ty tz):\t" << T_.topRightCorner(3, 1).transpose()
       << endl;

  cout << "\033[1;32mCalibrationResult\033[0m:\t"
       << "The calibrated transformation from camera to lidar is:" << endl;
  cout << "Transformation matrix: \n" << T_.inverse().matrix() << endl;
  Eigen::Matrix3d rot_m_c2l = T_.inverse().topLeftCorner(3, 3);
  cout << "Qaternion(qx qy qz qw):\t"
       << Eigen::Quaterniond(rot_m_c2l).coeffs().transpose() << endl;
  cout << "Translation(tx ty tz):\t"
       << T_.inverse().topRightCorner(3, 1).transpose() << endl;
...
void MainWindow::on_actionSave_Result_triggered() {
  ...
  for (auto &sensor_data : sensor_data_) {
    ...
    if (ply.img) {
      ...
      QMessageBox::warning(this, tr("Error"),
                           tr("img Id ") + QString("%1").arg(i) +
                               tr(" don't have features!"));
      f << js_.dump(4);
      f.close();
      return;
    }
    ...
    // point cloud
    fp = data_reader_->dataset_root_ + QDir::separator() +
         data_reader_->kPointcloudFolderName + QDir::separator() +
         QString("plane%1.pcd").arg(i);
  ...
  QMessageBox::information(this, tr("Success"),
                           tr("Saved to config.json successfully!\n"));
}

void MainWindow::openConfig(const string &config_path) {
  if (config_path.empty()) {
    cout << "Cofnig path is empty!" << endl;
    return;
  }
  ...
  ui->max_distance_slide->setValue(
      static_cast<int>(10 * flt["max_distance"].get<double>()));
  ui->min_distance_slide->setValue(
      static_cast<int>(10 * flt["min_distance"].get<double>()));
  ui->floor_gap_slide->setValue(
      static_cast<int>(10 * flt["floor_gap"].get<double>()));
  ui->max_distance_slide->setMaximum(
      static_cast<int>(10 * flt["max_distance_threshold"].get<double>()));
  ui->min_distance_slide->setMaximum(
      static_cast<int>(10 * flt["min_distance_threshold"].get<double>()));
  ui->floor_gap_slide->setMaximum(
      static_cast<int>(10 * flt["max_floor_gap"].get<double>()));
  ui->ceil_gap_slide->setValue(
      static_cast<int>(10 * flt["ceil_gap"].get<double>()));
  ui->ceil_gap_slide->setMaximum(
      static_cast<int>(10 * flt["max_ceil_gap"].get<double>()));

  ui->angle_start_text->setText(
      QString::number(ui->angle_start_slide->value()));
  ui->angle_size_text->setText(QString::number(ui->angle_size_slide->value()));
  ui->max_distance_text->setText(
      QString::number(flt["max_distance"].get<double>()));
  ui->min_distance_text->setText(
      QString::number(flt["min_distance"].get<double>()));
  ...
  connect(ui->angle_size_slide, &QSlider::valueChanged, this,
          &MainWindow::processSlider);
  connect(ui->max_distance_slide, &QSlider::valueChanged, this,
          &MainWindow::processSlider);
  connect(ui->min_distance_slide, &QSlider::valueChanged, this,
          &MainWindow::processSlider);
  connect(ui->floor_gap_slide, &QSlider::valueChanged, this,
          &MainWindow::processSlider);
  connect(ui->ceil_gap_slide, &QSlider::valueChanged, this,
          &MainWindow::processSlider);
}

void MainWindow::openDataset(const QString &dataset_path) {
  if (dataset_path.isEmpty()) {
    cout << "Dataset path is empty!" << endl;
    return;
  }
...
/**
 * @brief 將新来的一帧数据加入sensor_data_中，并预处理，open dataset和next
 * pose时会调用
 *
 * @param is_check
 * @return true
...
bool MainWindow::processData(bool is_check) {
  ...
  if (is_check && sensor_data_.size() > 0 && !sensor_data_.back().good() &&
      !skip_ids_.count(sid_)) // ensure that last frame is normal if last frame
                              // is not skipped.
  {
    QMessageBox::warning(this, tr("Error"),
                         tr("Current data is not good, adjust it!"));
    return false;
  }
  ...
  if (sensor_data_.size() > 0) {
    if (!data_reader_->moveNext()) {
      ...
      QMessageBox::information(
          this, tr("Success"),
          tr("Feature extraction completed!\nPlease click  Calibrate!"));
      return false;
    }
  }

  sensor_data_.emplace_back(
      data_reader_->getCurrentId()); // add a new data frame to sensor_data_
  sid_ = uint32_t(sensor_data_.size() - 1);
  auto &last = sensor_data_.back();
  last.img = data_reader_->getImage();
  ...
  // filter
  if (last.pc->size() >
      1000000) { // voxel grid downsample if the pc has very much points;
  ...
  getCorners(*last.img, K_, D,
             img_corners); // extract image corners by detecting ArUco Marker
  ply.img = std::make_shared<ImagePolygon>(4);
  if (img_corners[0].x != 0 && img_corners[0].y != 0) // 检测到aruco marker
  {
  ...
  // pc
  ply.pc =
      pcply_->Add(last.pc, last.pc_marked, last.pc_plane, last.pc_plane_coef);
...
void MainWindow::project(std::shared_ptr<cv::Mat> img_orig,
  ...
  for (auto &p : pc_proj_cam->points) {
    ...
    cv::circle(*img_proj, cv::Point2d(u, v), marker_redius,
...
void MainWindow::showCalibrateResult() {
  ...
  if (sd.img_proj == nullptr || sd.pc_proj == nullptr ||
    ...
    sort(sd.pc_proj_cam->points.begin(), sd.pc_proj_cam->points.end(),
  ...
  pcl::transformPointCloud(*sd.pc_proj_cam, *sd.pc_proj,
                           T_.inverse().cast<float>());
