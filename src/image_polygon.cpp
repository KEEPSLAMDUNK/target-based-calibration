...
void ImagePolygon::MarkImage(const cv::Mat& img, cv::Mat& img_out)
{
    ...
    for (uint32_t i = 0; i < size_; i++)
    {
        ...
        // draw vertex
        cv::circle(img_out, p0, 5, cv::Scalar(255,0,0),-1);
        // draw line
        cv::line(img_out,p0, p1, cv::Scalar(0,0,255) , 2);
        cv::putText(img_out, std::to_string(i+1),p0,cv::FONT_HERSHEY_SIMPLEX,1.2,CV_RGB(0,255,0),2);
    }
}
/*****************************END OF FILE**************************************/
